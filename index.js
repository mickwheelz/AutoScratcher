...
console.log("*** Welcome ***");
console.log(
  "This tool with set up a scratch org for you, based on the configuation in config.json in the root directory in which it runs"
);
console.log("We will create a scratch org with alias '" + sprintName + "'");
console.log(
  "Then push the source code from branch '" +
    config.gitBranch +
    "'" +
    " in repo '" +
    config.gitURL +
...
console.log(
  "Finally, it will run the apex code in '" +
    config.apexClassPath +
    "'" +
    " in the org"
);
...
console.log();
console.log(
  "Step 1 - Pulling source code from branch '" +
    config.gitBranch +
    "'" +
    " in repo '" +
    config.gitURL +
...
try {
  child_process.execSync(
    cloneRepo + branchOption + config.gitBranch + " " + config.gitURL
  );
} catch (e) {
...
console.log(
  "Step 2 - Creating new scratch org with alias '" + sprintName + "'"
);
...
if (resultJSON.status == 0) {
  console.log(
    "Org Successfully Created, Username: '" +
      resultJSON.result.username +
      "'" +
      ", Org Id: '" +
      resultJSON.result.orgId +
      "'"
  );
  ...
  console.log(
    "Step 3 - Pushing source code from: '" +
      config.sourcePath +
      "'" +
      " to org: '" +
      resultJSON.result.orgId +
  ...
  if (pushResultJSON.status == 0) {
    ...
    console.log(
      "Step 4 - Now executing apex class '" + config.apexClassPath + "'"
    );
    let execResultJSON = JSON.parse(
      child_process.execSync(
        executeApexCommand +
          usernameOption +
          resultJSON.result.username +
    ...
    if (execResultJSON.result.compiled == true || execResultJSON.result == 0) {
      console.log(
        "Apex class '" +
          config.apexClassPath +
          "' succesfully ran, your org is now ready!"
      );
      ...
      fs.writeFile("config.json", JSON.stringify(config), "utf8", function(
        ...
        if (err) {
          console.log(
            "*** Something went wrong updating config, details below ***"
          );
          console.log(err);
        } else {
          console.log(finishedMsg);
